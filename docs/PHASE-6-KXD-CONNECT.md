...
**Status:** Batches **C0** and **C1** implemented. Later batches (C2+) not authorized.  
...
## Experience north star (not implemented in C0/C1)
...
**These surfaces are not implemented in C0 or C1.** Do not present them as available. C2+ is not approved by this documentation.
...
**Not implemented:** polling UI or background polling loop.
...
| Metering primitives | ✅ | ✅ + message/conversation integration + Postgres atomic upsert |
| Audit events | ✅ | ✅ + conversation events |
| Conversations / participants / messages | ❌ | ✅ |
| Pagination / private unread | ❌ | ✅ |
| Admin messaging APIs | ❌ | ✅ (engine only) |
| Focused verifier | ✅ `verify:phase6-batch-c0` | ✅ `verify:phase6-batch-c1` |
| Connect UI / navigation | ❌ | ❌ |
| Realtime / presence / attachments | ❌ | ❌ |

---

## Explicit exclusions (C0 + C1)

Do not treat as implemented:

- Connect UI, shell, dock, app launcher, Buddy List, inbox
- Portal / client Connect surfaces
- Directory, presence, heartbeat, availability, away messages
...
3. Configure dogfood env (`KXD_CONNECT_ENABLED`, staff emails, org allowlist)
4. Confirm Postgres atomic meter path is active (not sqlite RMW fallback)
5. Run C0 + C1 verifiers and smoke the admin APIs with allowlisted staff
6. Keep kill switch available; do not enable production users
7. Do not ship Connect UI until a later authorized UX batch
...
2. Do not expose any Connect UI (none shipped in C0/C1)
3. If migrations must be reversed locally: run migration `down` for `20260816` then `20260815` only on non-production after backup
...
Not implemented. Not approved by this batch. Do not present messaging UI, directory, presence, or client Connect surfaces as available.
