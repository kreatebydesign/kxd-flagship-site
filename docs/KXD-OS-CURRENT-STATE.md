...
**Phase 6 — KXD Connect** — Batches **C0–C3** complete. Spec: `docs/PHASE-6-KXD-CONNECT.md`. Verifiers: `verify:phase6-batch-c0` … `verify:phase6-batch-c3`.
...
- C3: local dogfood readiness (atomic Postgres metering CTE, DB-native history/unread/mark-read, local fixtures). **Not** dogfood activation.
- No production migration, bootstrap, or Connect enablement authorized by C0–C3
...
4. **Phase 6 — KXD Connect** — Batches C0–C3 complete (foundation + messaging engine + staff UI + local dogfood readiness); dogfood activation and later batches not authorized. Spec: `docs/PHASE-6-KXD-CONNECT.md`. Keep disabled in production. Does **not** gate Founding Client Early Access.
...
| `PHASE-6-KXD-CONNECT.md` | Phase 6 Connect — Batches C0–C3 (tenancy / messaging engine / staff UI / local dogfood readiness); dock/dogfood activation excluded |
