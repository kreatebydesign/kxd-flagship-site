...
**Phase 6 — KXD Connect** — Batches **C0–C4** complete. Spec: `docs/PHASE-6-KXD-CONNECT.md`. Runbook: `docs/PHASE-6-CONNECT-LOCAL-DOGFOOD-RUNBOOK.md`. Verifiers: `verify:phase6-batch-c0` … `verify:phase6-batch-c4`.
...
- C4: local dogfood activation authorization (operator enable/disable, allowlist sync, immediate rollback, ops logging). **Not** production rollout.
- No production migration, bootstrap, or Connect enablement authorized by C0–C4
...
4. **Phase 6 — KXD Connect** — Batches C0–C4 complete (foundation + messaging engine + staff UI + local dogfood readiness + local activation controls). Controlled local dogfood only; production rollout and later batches not authorized. Spec: `docs/PHASE-6-KXD-CONNECT.md`. Keep disabled in production. Does **not** gate Founding Client Early Access.
...
| `PHASE-6-KXD-CONNECT.md` | Phase 6 Connect — Batches C0–C4 (tenancy / messaging / staff UI / local readiness / local activation); production enablement excluded |
