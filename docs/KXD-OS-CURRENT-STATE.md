...
**Phase 6 — KXD Connect** — Batches **C0–C5** complete. Spec: `docs/PHASE-6-KXD-CONNECT.md`. Runbook: `docs/PHASE-6-CONNECT-LOCAL-DOGFOOD-RUNBOOK.md`. Verifiers: `verify:phase6-batch-c0` … `verify:phase6-batch-c4` + `dogfood:connect-local`.
...
- C5: structured local dogfood operating period completed (multi-session, polling, rollback). **Not** production rollout.
- No production migration, bootstrap, or Connect enablement authorized by C0–C5
...
4. **Phase 6 — KXD Connect** — Batches C0–C5 complete (through structured local dogfood operating period). Production rollout and later batches not authorized. Spec: `docs/PHASE-6-KXD-CONNECT.md`. Keep disabled in production. Does **not** gate Founding Client Early Access.
...
| `PHASE-6-KXD-CONNECT.md` | Phase 6 Connect — Batches C0–C5 (through local dogfood operating period); production enablement excluded |
