...
**Phase 5 — Client Billing Visibility, Stripe Invoice Status & Monthly Work Summaries** — **Approved for phased implementation.** **Batches 5A and 5B implemented in repository and verified** (`verify:phase5-batch-5a`, `verify:phase5-batch-5b`). Batches **5C–5E not authorized**. Phase 5 as a whole is **not** complete. Clients cannot see invoices in the portal yet.
...
- **Batch 5B:** Stripe Invoice Read Foundation — server-only scoped list/read + allowlisted DTO (`lib/stripe/invoice-read-*.ts`); test-mode only; no portal UI
- Remaining batches: 5C Portal Billing Visibility → 5D Staff Invoice Visibility → 5E optional Billing/Work-Summary context
...
3. **Phase 5 — Client Billing Visibility, Stripe Invoice Status & Monthly Work Summaries** — Approved parallel non-Primal product lane. Spec: `docs/PHASE-5-CLIENT-BILLING-VISIBILITY.md`. **Batches 5A and 5B implemented/verified in repo** (`npm run verify:phase5-batch-5a`, `npm run verify:phase5-batch-5b`). Do **not** start Batches 5C–5E. Do not couple Phase 5 to Primal analytics or Batch J.2B.2.
...
| `PHASE-5-CLIENT-BILLING-VISIBILITY.md` | Phase 5 plan (Batches 5A–5B implemented/verified in repo; 5C–5E not authorized) |
