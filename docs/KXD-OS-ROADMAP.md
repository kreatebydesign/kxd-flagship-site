...
| **Phase 5** | **Client Billing Visibility, Stripe Invoice Status & Monthly Work Summaries** | **Batches 5A–5D implemented/verified in repo; 5E not authorized; phase not complete** | Spec: `docs/PHASE-5-CLIENT-BILLING-VISIBILITY.md`. Parallel non-Primal lane; does not complete or waive Phase 4. Batch 5A: monthly summary. Batch 5B: Stripe invoice read foundation. Batch 5C: portal Billing UI. Batch 5D: staff invoice visibility on Commercial Agreements detail (`verify:phase5-batch-5d`); TEST-mode read-only; no invoice management. Hosting Transitions and KXD invoice emails excluded. |
