...
| **Phase 6** | **KXD Connect** | **Batches C0–C1 complete** (tenancy / metering + secure messaging engine). Later batches not authorized. | Spec: `docs/PHASE-6-KXD-CONNECT.md`. C0: multi-organization foundation. C1: organization-owned conversations/messages, auth, pagination, private unread, metering — **no Connect UI**, realtime, or client exposure. **Does not block** KXD OS Founding Client Early Access. Separate readiness tracks: OS early access, Connect MVP, founding-company pilot, commercial Connect. |
...
**Phase 6 product outcome (C0–C1):** organization-owned Connect tenancy, membership, release controls, usage-metering primitives, and a secure messaging engine (conversations, messages, private unread, polling-ready APIs) — not a visible chat product. Client Communications, Connected Workspace, and `message-kxd` remain distinct and unchanged. Connect UI is deferred to a later authorized UX batch.
