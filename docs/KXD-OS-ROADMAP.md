...
| **Phase 6** | **KXD Connect** | **Batch C0 complete** (tenancy / release controls / metering primitives). Later batches not authorized. | Spec: `docs/PHASE-6-KXD-CONNECT.md`. Multi-organization Connect foundation only. No messaging UI, conversations, realtime, or client exposure. **Does not block** KXD OS Founding Client Early Access. Separate readiness tracks: OS early access, Connect MVP, founding-company pilot, commercial Connect. |
...
**Phase 6 product outcome (C0):** organization-owned Connect tenancy, membership, edition/feature release controls, and usage-metering primitives — foundation only; not a chat product. Client Communications, Connected Workspace, and `message-kxd` remain distinct and unchanged in C0.
