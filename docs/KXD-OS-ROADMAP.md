...
| **Phase 5** | **Client Billing Visibility, Stripe Invoice Status & Monthly Work Summaries** | **Batches 5A–5B implemented/verified in repo; 5C–5E not authorized; phase not complete** | Spec: `docs/PHASE-5-CLIENT-BILLING-VISIBILITY.md`. Parallel non-Primal lane; does not complete or waive Phase 4. Batch 5A: monthly summary (`verify:phase5-batch-5a`). Batch 5B: server-only Stripe invoice read foundation (`verify:phase5-batch-5b`); portal Billing UI remains Batch 5C+. Hosting Transitions and KXD invoice emails excluded. |
