...
| **Phase 5** | **Client Billing Visibility, Stripe Invoice Status & Monthly Work Summaries** | **Batches 5A–5C implemented/verified in repo; 5D–5E not authorized; phase not complete** | Spec: `docs/PHASE-5-CLIENT-BILLING-VISIBILITY.md`. Parallel non-Primal lane; does not complete or waive Phase 4. Batch 5A: monthly summary. Batch 5B: Stripe invoice read foundation. Batch 5C: portal Billing UI (`verify:phase5-batch-5c`); eligibility-gated nav; TEST-mode read-only; Stripe-hosted pay. Hosting Transitions and KXD invoice emails excluded. |
