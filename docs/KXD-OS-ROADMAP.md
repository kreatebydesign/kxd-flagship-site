...
| **Phase 6** | **KXD Connect** | **Batches C0–C3 complete** (foundation + messaging engine + staff UI + local dogfood readiness). Dogfood activation and later batches not authorized. | Spec: `docs/PHASE-6-KXD-CONNECT.md`. C0: tenancy. C1: secure messaging engine. C2: staff-only `/admin/connect` UI. C3: local Postgres metering/query readiness + fixtures (CP4). **Does not block** KXD OS Founding Client Early Access. |
...
**Phase 6 product outcome (C0–C3):** organization-owned Connect tenancy, secure messaging engine, staff-only `/admin/connect` UI, and proven local dogfood readiness (atomic metering, DB-native cursors) — not the full Connect shell (no dock/Buddy List/notifications). Dogfood activation and production enablement not authorized. Client Communications, Connected Workspace, and `message-kxd` remain distinct and unchanged.
