...
| **Phase 4** | **Multi-Client Portal Access & Account Context** | **Batches A–I in repository — code-complete; awaiting authenticated rollout QA** | Plan: `docs/PHASE-4-MULTI-CLIENT-PORTAL.md`. Identity: `docs/PHASE-4-PORTAL-IDENTITY-SECURITY.md`. Rollout: `docs/PHASE-4-PRODUCTION-ROLLOUT-CHECKLIST.md`. A–H as prior; I: invitations, roles, passkeys, TOTP (`verify:phase4-portal-identity-security`); migration unexecuted in production. Not fully production-complete until live multi-client + Don/Cusick + identity rollout QA. |
