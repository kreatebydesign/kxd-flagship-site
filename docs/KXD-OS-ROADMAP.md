...
| **Phase 6** | **KXD Connect** | **Batches C0–C2 complete** (foundation + messaging engine + staff UI). Later batches not authorized. | Spec: `docs/PHASE-6-KXD-CONNECT.md`. C0: tenancy. C1: secure messaging engine. C2: staff-only `/admin/connect` messaging UI (no dock/Buddy List/dogfood). **Does not block** KXD OS Founding Client Early Access. |
...
**Phase 6 product outcome (C0–C2):** organization-owned Connect tenancy, secure messaging engine, and a staff-only `/admin/connect` messaging UI — not the full Connect shell (no dock/Buddy List/notifications). Dogfood/production enablement not authorized. Client Communications, Connected Workspace, and `message-kxd` remain distinct and unchanged.
