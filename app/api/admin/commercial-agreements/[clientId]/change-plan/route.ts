/**
 * POST /api/admin/commercial-agreements/[clientId]/change-plan
...
import { changeCommercialPlan } from "@/lib/commercial-agreements/plan-change-service";
import { parsePlanChangeRequestBody } from "@/lib/commercial-agreements/plan-change-logic";
import {
  parseRouteClientId,
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  const parsed = parsePlanChangeRequestBody(body);
  if (!parsed.ok) {
    return NextResponse.json(
  ...
  try {
    ...
    const result = await changeCommercialPlan(clientId, {
    ...
    console.error("[KXD Commercial Plan Change] Change failed:", err);
    return NextResponse.json(
      { ok: false, message: "Unable to change commercial plan." },
      { status: 500 },
    );
  }
}
