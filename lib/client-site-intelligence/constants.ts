...
export const CSI_SALE_CONFIRMED_ACTIVITY_EVENT_TYPE =
  "client-site.sale.confirmed" as const;
export const CSI_COMMISSION_DUE_ACTIVITY_EVENT_TYPE =
  "client-site.commission.due" as const;
export const CSI_COMMISSION_PAID_ACTIVITY_EVENT_TYPE =
  "client-site.commission.paid" as const;
...
export const OTP_CARTS_INGEST_SECRET_ENV =
  "KXD_CSI_OTP_CARTS_INGEST_SECRET" as const;

/** Must never receive OTP Carts website ingest. */
export const ON_TRACK_PERFORMANCE_CLIENT_KEY = "otp" as const;

export const OTP_WEBSITE_LEAD_ID_PATTERN = /^OTP-WEB-\d{8}-[A-Za-z0-9]{6,16}$/;
...
export function buildCsiLifecycleActivitySourceId(
  eventRecordId: number,
  action: "sale-confirmed" | "commission-due" | "commission-paid",
): string {
  return `csi:event:${eventRecordId}:${action}`;
}
