...
function toTaskItem(doc: TaskDoc, ctx?: Awaited<ReturnType<typeof loadIntelligenceContext>>): TaskListItem {
  ...
  return {
    ...
    sourceType: doc.sourceType ? String(doc.sourceType) : doc.createdFrom ? String(doc.createdFrom) : null,
...
export async function getWorkPortfolio(): Promise<WorkPortfolioData> {
  ...
  return {
    ...
    dueThisWeek: open.filter(isDueThisWeek),
    ...
    stats: {
      ...
      dueThisWeekCount: open.filter(isDueThisWeek).length,
